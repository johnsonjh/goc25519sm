...
cleanUp()
{
	...
	set +e > /dev/null 2>&1 ||
		true :
	rm -f ./gocov_report_goc* > /dev/null 2>&1 ||
		true :
...
(
	...
	go test -v ${TEST1_TAGS:?${unset:?}} ${TEST_FLAGS:?${unset:?}} -bench=. ${GOC_TARGETS} -coverprofile gocov_report_goc1.profile |
		grep --line-buffered -v '^coverage: ' &&
		printf '%s\n' "Testing coverage with ${TEST2_TAGS:?${unset:?}}"
	# shellcheck disable=SC2086,SC2248
	go test -v ${TEST2_TAGS:?${unset:?}} ${TEST_FLAGS:?${unset:?}} -bench=. ${GOC_TARGETS} -coverprofile gocov_report_goc2.profile |
		grep --line-buffered -v '^coverage: ' &&
		gocovmerge gocov_report_goc1.profile gocov_report_goc2.profile > gocov_report_goc.profile &&
		/bin/rm -f -- ./gocov_report_goc1.profile ./gocov_report_goc2.profile &&
		gocov convert gocov_report_goc.profile > gocov_report_goc.json &&
...
if [ -x "${HOME}/.goc25519sm.cov.local" ]; then
	...
	("${HOME}/.goc25519sm.cov.local")
	printf '%s\n' "Local script ended"
fi

mkdir -p ./cov &&
	mv -f ./gocov_report_goc.* ./cov &&
